...
class Router {
		...
		for (; pathEnd <= pathLength; ++pathEnd) {
			if (pathEnd === pathLength || routePath[pathEnd] === "/") {
				...
				if (pathSegment[0] === ":") {
					...
					const paramMatch = pathSegment.match(/^:(.+)\((.+)\)$/); // Check for constraints
					if (paramMatch) {
						nextNode.paramName = paramMatch[1];
						nextNode.constraint = new RegExp(paramMatch[2]);
					} else {
						nextNode.paramName = pathSegment.substring(1);
					}
				} else {
					nextNode =
	...
	#printNode(node, prefix, level = 0, prefixSymbol = "") {
		...
		console.log(
			`${
				prefixSymbol ? `${indentation}${prefixSymbol} ${prefix || "/"}` : prefix
			}`
		);
	...
	async handleRequest(nativeReq, nativeRes) {
		...
		const routePath =
			queryDelimiter === -1 ? url : url.substring(0, queryDelimiter);
		...
		nativeReq.queryParams = new URLSearchParams(
			queryDelimiter === -1 ? "" : url.substring(queryDelimiter)
		);

		const routeHandlerFunc =
			routeHandler.requestHandler[routePath] || routeHandler.requestHandler;
	...
	#findRouteHandler(httpMethod, routePath) {
		...
		for (let pathEnd = 1; pathEnd <= pathLength; ++pathEnd) {
			if (pathEnd === pathLength || routePath[pathEnd] === "/") {
				...
				if (nextNode.constraint && !nextNode.constraint.test(pathSegment)) {
					return null; // Parameter doesn't match constraint
				}
