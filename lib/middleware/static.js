...
function isFresh(req, res) {
  ...
  if (ifNoneMatch) {
    ...
    const etag = getHeader(res._headers, 'etag');
    if (etag) {
  ...
  if (ifModifiedSince) {
    const lastModified = getHeader(res._headers, 'last-modified');
    if (lastModified) {
...
function createStaticMiddleware(root, options = {}) {
  ...
  // static(root, options) and static(options)
  if (typeof root === 'object' && !options.root) {
    options = root;
    root = options.root;
  }
  ...
  async function sendFile(req, res, filePath, stats) {
    ...
    // Check custom mime types first, then default ones
    let contentType;
    if (options.mimeTypes && options.mimeTypes[ext]) {
      contentType = options.mimeTypes[ext];
    } else if (options.mimeTypes && options.mimeTypes[extNoDot]) {
      contentType = options.mimeTypes[extNoDot];
    } else {
      contentType = MIME_TYPES[extNoDot] || 'application/octet-stream';
    }
  ...
  return async function staticMiddleware(req, res, next) {
    ...
    try {
      ...
      // If mounted with a path prefix (e.g., app.use('/static', staticMiddleware))
      // strip the baseUrl from the pathname
      if (req.baseUrl && pathname.startsWith(req.baseUrl)) {
        pathname = pathname.slice(req.baseUrl.length);
      }
