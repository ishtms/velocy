  ...
  async handleFile(fieldName, filename, content, headers) {
    ...
    if (content.length > this.options.fileMemoryLimit) {
      ...
      const tempPath = path.join(
        ...
        `upload_${crypto.randomBytes(16).toString('hex')}`
      );
...
function bodyParser(options = {}) {
  ...
  const config = {
    json: true,
    jsonLimit: parseSize(options.jsonLimit || '100kb'),
    urlencoded: true,
    urlencodedLimit: parseSize(options.urlencodedLimit || '100kb'),
    extended: options.extended !== false,
    parameterLimit: options.parameterLimit || 1000,
    multipart: true,
    multipartLimit: parseSize(options.multipartLimit || '10mb'),
    fileMemoryLimit: parseSize(options.fileMemoryLimit || '1mb'),
    tempDirectory: options.tempDirectory || os.tmpdir(),
    raw: false,
    rawLimit: parseSize(options.rawLimit || '100kb'),
  ...
  return async function bodyParserMiddleware(req, res, next) {
    ...
    try {
      ...
      if (body !== null) {
        // Override the body getter to return parsed body
        ...
        // Also set a synchronous property for convenience
        req.parsedBody = body;
