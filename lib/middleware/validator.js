...
function createFieldValidator(source) {
  return function(fieldOrSchema) {
    // If an object is passed, treat it as a schema for multiple fields
    if (typeof fieldOrSchema === 'object' && fieldOrSchema !== null) {
      // Return a middleware that validates the entire object
      return async function(req, res, next) {
        const errors = [];
        
        // Get the value to validate based on source
        let sourceData;
        switch (source) {
          case 'body':
            // Handle the case where req.body might be a Promise
            if (req.body && typeof req.body.then === 'function') {
              sourceData = await req.body;
            } else {
              sourceData = req.body;
            }
            break;
        ...
        for (const [field, fieldSchema] of Object.entries(fieldOrSchema)) {
          const value = sourceData[field];
          const result = validateValue(value, fieldSchema, `${source}.${field}`);
          
          if (!result.valid) {
            errors.push(...result.errors);
          } else if (result.value !== undefined && source === 'body') {
            // Update the validated value
            if (!req.parsedBody) {
              req.parsedBody = { ...sourceData };
            }
            req.parsedBody[field] = result.value;
          }
        }
    ...
    // Otherwise, treat it as a single field name (original behavior)
    const validator = new FieldValidator(source, fieldOrSchema);
