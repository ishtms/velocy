...
const TypeValidators = {
  ...
  array(value, options = {}) {
    ...
    if (options.min != null && value.length < options.min) {
      return { valid: false, error: `Array must have at least ${options.min} items` };
    }

    if (options.max != null && value.length > options.max) {
      return { valid: false, error: `Array must have at most ${options.max} items` };
    }
    ...
    try {
      ...
      if (options.protocols) {
        ...
        if (!options.protocols.includes(protocol)) {
          return { valid: false, error: `URL protocol must be one of: ${options.protocols.join(', ')}` };
        }
      }
