...
function createRateLimiter(options = {}) {
  ...
  async function rateLimitMiddleware(req, res, next) {
    ...
    if (distributedLimiter) {
      ...
      const limiter = store.get(key, config.algorithm, config);
      
      // Check rate limit based on algorithm
      if (config.algorithm === 'token-bucket') {
        result = limiter.consume(config.cost || 1);
      } else {
        result = limiter.increment();
      }
    }
    ...
    if (config.skipSuccessfulRequests || config.skipFailedRequests) {
      ...
      res.end = function(...args) {
        // Check if we should retroactively not count this request
        ...
        if ((config.skipSuccessfulRequests && wasSuccessful) ||
          ...
          // We need to "refund" the request
          // This is tricky with the current architecture
          // For token bucket, we could add tokens back
          // For counters, we could decrement
          // This would require modifying the limiter classes
          // For now, we'll leave this as a limitation
        }
