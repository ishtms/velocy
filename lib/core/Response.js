...
class Response {
   ...
   * Sets Content-Length for better HTTP/1.1 performance unless already set.
   * 
  ...
  json(data) {
    ...
    // Set Content-Length for better performance if not already set
    // This improves HTTP/1.1 keep-alive efficiency
    // Compression middleware will override this if needed
    if (!this.#headers['Content-Length'] && !this.#headersSent) {
  ...
  send(data) {
    if (data === null || data === undefined) {
      // Set Content-Length for empty response
      if (!this.#headers['Content-Length'] && !this.#headersSent) {
      ...
      if (!this.#nativeResponse.headersSent) {
        ...
        // Set Content-Length for better performance if not already set
        // CRITICAL: Use Buffer.byteLength for UTF-8, not string.length!
      ...
      if (Buffer.isBuffer(data)) {
        ...
        // Set Content-Length for buffers (simple length property)
        if (!this.#headers['Content-Length'] && !this.#headersSent) {
          this.set('Content-Length', data.length.toString());
        }
        this.end(data);
      } else {
        // Send as JSON (json() will handle Content-Length)
        this.json(data);
      }
    } else {
      // Convert to string for other types
      const str = String(data);
      // Set Content-Length for converted strings
      if (!this.#headers['Content-Length'] && !this.#headersSent) {
  ...
  redirect(url, status = 302) {
    ...
    // Set Content-Length: 0 for empty redirect response
    if (!this.#headers['Content-Length']) {
      this.set('Content-Length', '0');
    }
    this.status(status);
   ...
   * Note: Using write() disables automatic Content-Length.
   * 
   ...
   */
  write(...args) {
    // Mark that streaming is being used - don't set Content-Length
    this._isStreaming = true;
    
    // Ensure headers are sent before writing
    if (!this.#nativeResponse.headersSent) {
      // Remove Content-Length if it was set (streaming response)
      if (this.#headers['Content-Length']) {
